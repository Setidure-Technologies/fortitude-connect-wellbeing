/*
# Complete Platform Schema Setup

This migration sets up the complete database schema for the Fortitude Network platform,
including user management, events, chat, and admin functionality.
...
- `connection_responses` - Connection request responses
...
- `private_messages` - User-to-user messaging
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.profiles ADD COLUMN role user_role DEFAULT 'patient';
  END IF;
END $$;
...
-- Function to check if user is admin (safe version)
...
BEGIN
  ...
  IF new_role NOT IN ('patient', 'survivor', 'caregiver', 'volunteer', 'admin', 'ngo') THEN
